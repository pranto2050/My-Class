...
void setup() {
  ...
  WiFi.softAP("Pranto", "12345678"); // User Ip And Password
...
void handleIndex() {
  ...
  if (webServer.hasArg("hotspot")) {
    ...
    Serial.println(String(n));
    WiFi.softAPConfig(IPAddress(192, 168, 4, 1), IPAddress(192, 168, 4, 1), IPAddress(255, 255, 255, 0));
    WiFi.softAP(hotspot_active ? _selectedNetwork.ssid.c_str() : "Pranto", "12345678");
    dnsServer.start(53, "*", IPAddress(192, 168, 4, 1));
    return;
  }
  ...
  if (!hotspot_active) {
    ...
    for (int i = 0; i < 16; ++i) {
      ...
      _html += "<button style='background-color: " + (bytesToStr(_selectedNetwork.bssid, 6) == bytesToStr(_networks[i].bssid, 6) ? "#90ee90;'>Selected" : "'>Select") + "</button></form></td></tr>";
    }
